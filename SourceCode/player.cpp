...
float m_JPcharge;
bool spaceKayHold;
const float MaxJamp = 5.5f;
const float jumpChargeRate = 0.088f;

bool isRunning = false; // 走行状態を管理する変数
...
void player_update()
{
    ...
    case 0:
        ...
        sprPlayer = sprite_load(L"./Data/Images/tobiuoman.png");
    ...
    case 1:
        ...
        player              = {};
        player.timer        = 0;
        player.pos          = { 1100.0f , SCREEN_H * 0.5f };
        player.scale        = { 1.5f, 2.0f };
        player.texPos       = { 0, 0 };
...
bool player_jump()
{
   /* if (TRG(0) & PAD_TRG1 && player.jumpCount > 0 && player.speed.y > 0)
    ...
    if (STATE(0) & PAD_TRG1)
    {
        spaceKayHold = true; // 長押し開始
        power += jumpChargeRate;
        if (power > MaxJamp)
        {
            power = MaxJamp;
        }
    ...
    // スペースキーが離されたとき
    else if (TRG_RELEASE(0) & PAD_TRG1)
    {
        if (spaceKayHold && player.jumpCount > 0)
        {
            sound::play(XWB_STAGE1, XWB_STAGE1_JUMP);
            // 大ジャンプ
            if (power >= MaxJamp) // 大ジャンプの基準を設定（20.0fは例）
            {
                player.speed.y = -PLAYER_JUMP_POWER_HIGH * power;
            }
            else if (power < MaxJamp && power >= 0.5f)
            {
                player.speed.y = -PLAYER_JUMP_POWER_CYU * power;
            }
            else // 通常ジャンプ
            {
            ...
            player.jumpEnd = false;
            power = 0.0f;
            spaceKayHold = false; // 長押し終了
            return true;
        }
    }
    else if (STATE(0) & PAD_TRG1)
    {
        // スペースキーがまだ押されている場合、長押し状態を維持
        spaceKayHold = true;
    }
    return false;
}
